...
class FigureSupplier {
    ...
    fun getRandomFigure(): Figure {
        val randomFigure : FigureEnum
        = FigureEnum.values()[Random.nextInt(FigureEnum.values().size)]
        val firstRandomNumber = BigDecimal(Random.nextDouble(DEFAULT_RANGE))
            .setScale(DEFAULT_SCALE, RoundingMode.HALF_EVEN).toDouble()
        val secondRandomNumber = BigDecimal(Random.nextDouble(DEFAULT_RANGE))
            .setScale(DEFAULT_SCALE, RoundingMode.HALF_EVEN).toDouble()

        return when (randomFigure) {
            FigureEnum.CIRCLE -> Circle(colorSupplier.getRandomColor().toString().lowercase(),
                firstRandomNumber)

            FigureEnum.ISOSCELES_TRAPEZOID
            -> IsoscelesTrapezoid(colorSupplier.getRandomColor().toString().lowercase(),
